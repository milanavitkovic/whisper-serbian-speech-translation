...
"""
Serbian → English Speech Translation with Whisper
...
Validation Data:
    - Južne Vesti (validation split)

Test Data:
    - Južne Vesti (test split)
...
# Data paths for Južne Vesti dataset
JUZNE_VESTI_PATHS = {
...
def prepare_dataset(
    ...
    """
    if verify_audio:
        print("🔍 Verifying audio file existence...")
        valid_data = []
        missing_count = 0
        ...
        for d in data:
            if os.path.exists(d["audio"]):
                ...
                if missing_count <= 5:  # Show first 5 missing files
                    print(f"⚠️  File not found: {d['audio']}")

        if missing_count > 0:
            print(f"⚠️  Total missing files: {missing_count}")
            print(f"✅ Valid files found: {len(valid_data)}")
...
if __name__ == "__main__":
    ...
    print("=" * 70)
    print("Serbian → English Speech Translation - Whisper Fine-tuning")
    print("=" * 70)
    print(f"\nModel: {MODEL_NAME}")
    ...
    # Load Južne Vesti train set
    print("\n📦 Loading Južne Vesti train set...")
    ...
    print("📦 Loading ParlaSpeech train set...")
    ...
    print(f"✅ Combined training data: {len(jv_train_data)} (JV) + {len(ps_train_data)} (PS) = {len(train_data)}")

    # Load Južne Vesti validation set
    print("\n📦 Loading Južne Vesti validation set...")
    ...
    # Load Južne Vesti test set
    print("📦 Loading Južne Vesti test set...")
    ...
    # Configure model for Serbian → English translation
    ...
    print(f"✅ Configured for: Serbian → English translation")
    ...
    print("\n🧪 Testing model on a sample from test set...\n")
